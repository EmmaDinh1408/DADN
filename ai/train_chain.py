"""
MechDrive AI — Train Chain Agent
===================================
Script chạy train Q-Learning cho bộ truyền xích.

Usage:
    python train_chain.py [--epochs N] [--quiet]
"""
...
from config import EPOCHS_CHAIN, P_BUCKETS, N_BUCKETS, LOAD_TYPES
from agents.chain_agent import ChainAgent


def main():
    parser = argparse.ArgumentParser(description="Train Chain Q-Learning Agent")
    parser.add_argument("--epochs", type=int, default=EPOCHS_CHAIN, help="Number of training epochs")
    parser.add_argument("--quiet", action="store_true", help="Suppress per-epoch output")
    args = parser.parse_args()

    print("=" * 70)
    print("  MechDrive AI — CHAIN AGENT TRAINING")
    print("=" * 70)
    print(f"  Epochs:       {args.epochs:,}")
    print(f"  Action space: {len(ChainAgent().all_actions)} actions")
    ...
    for P, n, load, desc in samples:
        ...
        if "error" not in result:
            print(
                f"  {desc:40s} → "
                f"u_x={result['optimal_ux']}, "
                f"z₁={result['optimal_z1']}, "
    ...
    import json
    curve_path = os.path.join(os.path.dirname(__file__), "output", "chain_reward_curve.json")
